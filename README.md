...
The Module B report at [`artifacts/module_b_acceptance_evaluation.json`](artifacts/module_b_acceptance_evaluation.json) uses the repository's newer 5%-defect dataset and the same lot-safe split. Its unseen test partition records 0.293 aggregate MAE versus 0.437 for persistence and 1.342 for linear extrapolation, detects 14/14 injected defective components with zero false negatives, and has an 8.19% healthy-component flag rate. Aggregate MAE mixes configured parameter units, so the report also provides per-parameter MAE. These are synthetic software-acceptance results, not hardware qualification claims.
